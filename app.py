...
# ─────────────────────────────────────────────
# TOTP secrets saved to file so they survive restarts
# ─────────────────────────────────────────────
...
def send_sms(phone, otp):
    ...
    client.messages.create(body=f'Your MFA code: {otp}', from_=TWILIO_FROM, to=phone)
    ms = round((time.time() - t1) * 1000, 2)
    print(f'[MEASUREMENT] SMS   | send_duration: {ms} ms')
    return ms
...
def mfa_verify():
    ...
    if request.method == 'POST':
        ...
        if method == 'totp':
            ...
            print(f'[MEASUREMENT] TOTP  | send_duration: N/A (generated locally) | result: {result}')
            if not ok:
            ...
            if time.time() - ts > 300:
                ...
                ok = True
            result = 'SUCCESS' if ok else 'FAIL'
            print(f'[MEASUREMENT] {method.upper():<5} | result: {result}')
