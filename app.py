...
def send_sms(phone, otp):
    ...
    # Save sent_at for delivery time calculation in callback
...
def twilio_status():
    ...
    # Only print delivery time when status is "delivered"
...
def verify():
    ...
    if request.method == 'POST':
        ...
        if method == 'totp':
            # Measure verification processing time for TOTP
            t1 = time.time()
            ok = verify_totp_code(username, code)
            verification_ms = round((time.time() - t1) * 1000, 2)
            print(f'[MEASUREMENT] TOTP | verification_processing_time_ms: {verification_ms}')
            if not ok:
                error = 'Invalid code. Check your authenticator app.'

        else:
            # Measure verification processing time for SMS/Email
            t1 = time.time()
            if time.time() - session.get('otp_ts', 0) > 300:
                ...
                error = 'Wrong code.'
            verification_ms = round((time.time() - t1) * 1000, 2)
            print(f'[MEASUREMENT] {method.upper()} | verification_processing_time_ms: {verification_ms}')
